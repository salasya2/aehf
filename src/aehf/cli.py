...
@app.command()
def run(path: Path, agent : AgentChoice, tools: ToolChoice, n_samples:int = 1,  concurrency: int = 5, max_tokens : int = 1024, recordings_dir : Path = Path("output/"), model : str = "claude-haiku-4-5-20251001", prompt_version:str = "v1",judge_model :str = "claude-haiku-4-5-20251001",judgechoice : JudgeChoice = JudgeChoice.assertion, output :Path | None = None) ->None:
    load_dotenv()
    try:
    ...
    judge = build_judge(judgechoice,"v1",judge_model,max_tokens)
